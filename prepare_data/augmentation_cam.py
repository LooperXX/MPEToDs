...
entity_types = ['@address', '@area', '@food', '@id', '@location', '@name', '@phone', '@postcode', '@pricerange', '@type'] # @name | @address @area @id @location @phone @postcode

# candidate types: entity types that can be used for augmentation, filter by manual check
candidate_types = ['address', 'area', 'id', 'location', 'phone', 'postcode']
count = 0
repeat_time = 10
domains = {'#restaurant#': []}
...
with open('./data/fine-tune/CamRest676/CamRest676_entities.json') as f:
...
with open('./data/fine-tune/CamRest676/train.txt') as fin:
    for line in fin:
        line = line.strip()
        if line:
            if line[-1] == line[0] == '#':
...
random.shuffle(dev)
os.makedirs('./data/pre-train/CamRest676', exist_ok=True)
with open('./data/pre-train/CamRest676/train_{}.txt'.format(str(repeat_time)), 'w') as fw:
    for lines in train:
        fw.write('\n'.join(lines))
        fw.write('\n\n')

with open('./data/pre-train/CamRest676/dev_{}.txt'.format(str(repeat_time)), 'w') as fw:
    for lines in dev:
        fw.write('\n'.join(lines))
        fw.write('\n\n')